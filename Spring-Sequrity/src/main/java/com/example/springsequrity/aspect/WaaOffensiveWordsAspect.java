package com.example.springsequrity.aspect;

import com.example.springsequrity.aspect.data.OffensiveRequest;
import com.example.springsequrity.aspect.data.OffensiveWordDictionary;
import com.example.springsequrity.exceptions.MaxBadWordLimitException;
...
public class WaaOffensiveWordsAspect {
    ...
    @Before("execution(* com.example.springsequrity.controller.ProductController.*(..))")
    public void logOffensiveWords(JoinPoint joinPoint) {
        ...
        if (bannedUsers.containsKey(userName) && bannedUsers.get(userName) <= System.currentTimeMillis()) {
            throw new MaxBadWordLimitException(String.format("Max Bad Words Requests Limit has been Reached. You need wait for %d minutes.",
